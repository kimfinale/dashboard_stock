...
def fetch_remote_data(url):
    try:
        ...
        # If fetching fails, output error to stderr but maybe fall back to mock?
        # For now, let's just fail loudly so we know.
        import sys
        sys.stderr.write(f"Error fetching remote data from {url}: {e}\n")
        exit(1)

if __name__ == "__main__":
    import os
    # If the user provides a data URL via environment variable (e.g. in GitHub Actions), use it.
    # Otherwise, generate mock data for local development.
    data_url = os.environ.get("PORTFOLIO_DATA_URL")
    
    if data_url:
        fetch_remote_data(data_url)
    else:
        generate_mock_data()
